...
class ComprehensiveValidator:
    ...
    def test_apis(self):
        ...
        endpoints = [
            ...
            ("ShadowSearch (SearX)", "http://localhost:8002")  # Added ShadowSearch
        ]
        ...
        return {
            'status': 'PASS' if all('✓' in r for r in results) else 'WARNING',
            'details': f"API Status: {' | '.join(results)}"
        }

    def test_services(self):
        """Test all running services including ShadowSearch"""
        services = [
            (8000, "REST API"),
            (8003, "Threat API"),
            (8004, "Main API"),
            ...
            (8080, "Proxy"),
            (8083, "WebSocket"),
            (8002, "ShadowSearch"),  # Added ShadowSearch
            (9090, "Threat Insight"),
            (5432, "PostgreSQL"),
            (6379, "Redis"),
            (7474, "Neo4j HTTP"),
    ...
    def test_shadowsearch(self):
        """Test ShadowSearch (SearX) functionality"""
        try:
            # Check if ShadowSearch is running
            response = requests.get('http://localhost:8002', timeout=5)
            if response.status_code < 500:
                # Check for SearX specific content
                if 'searx' in response.text.lower() or 'search' in response.text.lower():
                    return {
                        'status': 'PASS',
                        'details': f"ShadowSearch running (HTTP {response.status_code}) - Private search engine operational"
                    }
                else:
                    return {
                        'status': 'WARNING',
                        'details': f"ShadowSearch responding but may not be SearX instance"
                    }
            else:
                return {
                    'status': 'FAIL',
                    'error': f"ShadowSearch HTTP {response.status_code}"
                }
        except Exception as e:
            return {'status': 'FAIL', 'error': str(e)}
    ...
    def run_all_tests(self):
        ...
        tests = [
            ...
            ("ShadowSearch", self.test_shadowsearch),  # Added ShadowSearch test
    ...
    def print_summary(self):
        ...
        print(f"\nOverall Score: {color}{score:.1f}%{reset_code}")
        print(f"Grade: {color}{grade}{reset_code}")
        ...
        if score >= 90:
            ...
            print("  • Use ShadowSearch for private threat research")
        elif score >= 75:
...
async def quick_validation():
    ...
    # 5. Check ShadowSearch
    try:
        response = requests.get('http://localhost:8002', timeout=5)
        if response.status_code < 500:
            checks.append("✅ ShadowSearch: Running")
        else:
            checks.append(f"❌ ShadowSearch: HTTP {response.status_code}")
    except:
        checks.append("❌ ShadowSearch: Not reachable")

    # 6. Check recent reports
    import glob
    reports = glob.glob('/opt/shadowcore/intelligence_reports/*.json')
    ...
    if all('✅' in check for check in checks[:5]):
